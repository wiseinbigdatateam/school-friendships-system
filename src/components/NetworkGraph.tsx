...
const NetworkGraph: React.FC<NetworkGraphProps> = ({
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation<D3Node>(nodes)
      .force('link', d3.forceLink<D3Node, any>(edges).id((d: D3Node) => d.id).distance(100))
    ...
    // 호버 효과
    node.on('mouseover', function(event, d: D3Node) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center">
      <div className="mb-4 text-center">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">개인별 네트워크 시각화</h3>
        <p className="text-sm text-gray-600">
          {isInteractive ? '노드를 드래그하여 이동하고, 줌하여 확대/축소할 수 있습니다.' : '읽기 전용 모드'}
        </p>
      </div>
