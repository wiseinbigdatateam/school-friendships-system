        ...
        <div className="flex justify-between items-center h-12">
          ...
          <div className="flex items-center">
            <button
              ...
              className="flex items-center space-x-2 text-blue-600 font-bold text-lg hover:text-blue-700 transition-colors"
            >
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center space-x-1 px-3 py-2 text-sm font-medium rounded-md transition-colors ${
                  isCurrentPath('/dashboard') || isCurrentPath('/survey-templates') || isCurrentPath('/survey-management')
              ...
              {activeDropdown === 'friendship' && (
                <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-50">
                  <div className="py-1">
                    ...
                    <button
                      onClick={() => handleNavigationClick('/survey-management')}
                      className={`block w-full text-left px-4 py-2 text-sm transition-colors ${
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center space-x-1 px-3 py-2 text-sm font-medium rounded-md transition-colors ${
                  isCurrentPath('/network-analysis') || isCurrentPath('/individual-analysis')
                    ? 'bg-blue-50 text-blue-600'
                    : 'text-gray-700 hover:text-blue-600 hover:bg-gray-50'
                }`}
              >
              ...
              >
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500 truncate max-w-20">
                    {user?.role === 'homeroom_teacher' ? '담임교사' :
                     user?.role === 'school_admin' ? '학교 관리자' :
                     user?.role === 'grade_teacher' ? '학년 부장' :
                     user?.role === 'district_admin' ? '교육청 관리자' :
                     user?.role === 'main_admin' ? '메인 관리자' : '사용자'}
                  </p>
                </div>
