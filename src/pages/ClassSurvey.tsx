    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const surveysWithTemplates = data.map((survey: any) => {
          const template = templates.find((t: any) => t.id === survey.template_id);
          return {
            ...survey,
            survey_templates: template ? { metadata: template.metadata } : undefined
          };
        });
  ...
  const fetchChartData = async () => {
    try {
      ...
      if (responsesData && surveyData && surveyData.questions) {
        ...
        const chartDataArray: ChartData[] = questions.map((question: any, index: number) => {
          // 답변 옵션이 있는 경우 (학교폭력, 만족도 설문)
          if (question.answer_options) {
            const yesCount = responsesData.filter((response: any) => 
              response.responses && response.responses[`q${index + 1}`] === '1'
            ).length;
            
            const noCount = responsesData.filter((response: any) => 
              response.responses && response.responses[`q${index + 1}`] === '2'
            ).length;
            ...
            const yesCount = responsesData.filter((response: any) => 
              response.responses && response.responses[`q${index + 1}`] && 
            ...
            {surveys.map((survey) => (
              ...
              >
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  <p>카테고리 : {survey.survey_templates?.metadata?.category || '알 수 없음'}</p>
                  <p>상태 : <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusStyle(survey.status)}`}>
                    {getStatusLabel(survey.status)}
                  </span></p>
                  <p>생성일 : {survey.created_at ? formatDate(survey.created_at) : ''}</p>
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  {chartData.map((data, index) => (
                    <div key={index} className="bg-white rounded-lg border border-gray-200 p-6">
                      ...
                      {/* 차트 제목 */}
                      <div className="mb-4">
                        <h4 className="text-sm font-medium text-gray-700 mb-3">응답 분포</h4>
                      </div>
                      ...
                      <div className="flex items-end space-x-6 h-48 border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <div className="flex flex-col items-center">
                          <div 
                            ...
                            style={{ 
                              height: `${Math.max((data.yes_count / Math.max(data.yes_count + data.no_count, 1)) * 180, 30)}px`,
                              minHeight: '30px'
                            }}
                          ></div>
                          <span className="text-sm text-gray-600 mt-2">예</span>
                          <span className="text-lg font-semibold text-blue-600">{data.yes_count}</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center">
                          <div 
                            ...
                            style={{ 
                              height: `${Math.max((data.no_count / Math.max(data.yes_count + data.no_count, 1)) * 180, 30)}px`,
                              minHeight: '30px'
                            }}
                          ></div>
                          <span className="text-sm text-gray-600 mt-2">아니오</span>
                          <span className="text-lg font-semibold text-blue-700">{data.no_count}</span>
                        </div>
                      </div>
                      
                      {/* 디버깅 정보 */}
                      <div className="mt-2 text-xs text-gray-500">
                        <p>차트 데이터: 예 {data.yes_count}명, 아니오 {data.no_count}명</p>
                      </div>
                  ...
                  {chartData.map((data, index) => (
                    <div key={index} className="bg-white rounded-lg border border-gray-200 p-6">
                      ...
                      <div className="flex gap-8">
                        ...
                        <div className="flex-1">
                          <div className="mb-4">
                            <h4 className="text-sm font-medium text-gray-700 mb-3">응답 현황</h4>
                          </div>
                          ...
                          <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
                            <table className="w-full">
                              <thead className="bg-gray-100">
                                <tr>
                                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">답변</th>
                                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">학생</th>
                                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">합계</th>
                                </tr>
                              </thead>
                              <tbody className="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                  <td className="px-4 py-3">
                                    <div className="flex items-center space-x-2">
                                      <div className="w-3 h-3 bg-blue-500 rounded"></div>
                                      <span className="text-sm font-medium text-gray-900">예</span>
                                    </div>
                                  </td>
                                  <td className="px-4 py-3">
                                      ...
                                      {data.yes_students && data.yes_students.length > 0 ? (
                                        data.yes_students.map((name, i) => (
                                          <span key={i} className="text-xs text-gray-600">
                                            {name}{i < (data.yes_students?.length || 0) - 1 ? ',' : ''}
                                          </span>
                                        ))
                                      ) : (
                                  ...
                                  <td className="px-4 py-3">
                                    <span className="text-sm font-semibold text-blue-600">{data.yes_count}명</span>
                                  </td>
                                </tr>
                                ...
                                <tr>
                                  <td className="px-4 py-3">
                                    <div className="flex items-center space-x-2">
                                      <div className="w-3 h-3 bg-blue-700 rounded"></div>
                                      <span className="text-sm font-medium text-gray-900">아니오</span>
                                    </div>
                                  </td>
                                  <td className="px-4 py-3">
                                    <div className="flex flex-wrap gap-1">
                                      {data.no_students && data.no_students.length > 0 ? (
                                        data.no_students.map((name, i) => (
                                          <span key={i} className="text-xs text-gray-600">
                                            {name}{i < (data.no_students?.length || 0) - 1 ? ',' : ''}
                                          </span>
                                        ))
                                      ) : (
                                        <span className="text-xs text-gray-500">-</span>
                                  ...
                                  <td className="px-4 py-3">
                                    <span className="text-sm font-semibold text-blue-700">{data.no_count}명</span>
                                  </td>
                                </tr>
                              </tbody>
                            </table>
