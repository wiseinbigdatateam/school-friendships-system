...
const SurveyResponse: React.FC = () => {
  ...
  const [surveyTemplate, setSurveyTemplate] = useState<SurveyTemplate | null>(null); // 템플릿 정보 추가
  
  // 학생 본인 확인 상태
  const [currentStep, setCurrentStep] = useState<'verify' | 'survey' | 'complete' | 'already_responded'>('verify');
      ...
      try {
        ...
        if (surveyData) {
          ...
          if (surveyData.questions && Array.isArray(surveyData.questions)) {
            console.log('질문별 max_selections:', surveyData.questions.map((q: any) => ({
              id: q.id,
        ...
        surveyData.questions.forEach((q: any, index: number) => {
          ...
          // 2. 설문 대상 학생들 로드
          if (surveyData.target_grades && surveyData.target_classes && surveyData.school_id) {
  ...
  const handleStudentVerification = async () => {
    ...
    if (matchedStudent) {
      ...
      setVerificationError('일치하는 학생 정보를 찾을 수 없습니다. 이름과 생년월일을 다시 확인해주세요.');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 완료 알림 표시
      alert(`🎉 ${selectedStudent.name}님, 설문 응답이 완료되었습니다!\n\n📝 응답 내용이 성공적으로 저장되었습니다.\n👋 감사합니다!`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">설문을 찾을 수 없습니다</h1>
          <p className="text-gray-600 mb-6">{error || '요청하신 설문이 존재하지 않습니다.'}</p>
          <button
            onClick={() => navigate('/')}
        ...
        <div className="max-w-2xl mx-auto px-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="text-sm text-gray-500">
              <p>기간: {survey.start_date} ~ {survey.end_date}</p>
              <p>대상: {survey.target_grades?.join(', ')}학년 {survey.target_classes?.join(', ')}반</p>
            </div>
          </div>
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-gray-500">{selectedStudent.grade}학년 {selectedStudent.class}반</p>
              </div>
            </div>
            <div className="text-sm text-gray-500">
              <p>기간: {survey.start_date} ~ {survey.end_date}</p>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            {survey.questions && Array.isArray(survey.questions) && survey.questions.map((question: any, index) => (
                    ...
                    {surveyTemplate?.metadata?.category === '교우관계' ? (
                                    ...
                                    onChange={(e) => {
                                      if (e.target.checked) {
                                        ...
                                        if (currentValues.length < maxSelections) {
                                          handleResponseChange(question.id, [...currentValues, student.id]);
                                        }
                                      } else {
                      ...
                      <>
                        ...
                        <div className="space-y-3">
                          {question.answer_options && Object.entries(question.answer_options).map(([key, value]) => (
                            <label
                              key={key}
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center max-w-md w-full">
          ...
          <h2 className="text-2xl font-bold text-blue-600 mb-3">📝 이미 응답 완료!</h2>
          <p className="text-lg text-gray-700 mb-4">
            <span className="font-semibold text-blue-600">{selectedStudent.name}</span>님은 
            이미 이 설문에 응답하셨습니다.
          </p>
          <div className="text-sm text-gray-600 mb-6 space-y-2">
            <p className="flex items-center justify-center">
              <span className="mr-2">⏰</span>
              응답 시간: {new Date(existingResponse.submitted_at).toLocaleString('ko-KR')}
            </p>
            <p className="flex items-center justify-center">
              <span className="mr-2">✅</span>
              응답 내용이 안전하게 저장되었습니다
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center max-w-md w-full">
          ...
          <h2 className="text-2xl font-bold text-green-600 mb-3">🎉 설문 완료!</h2>
          <p className="text-lg text-gray-700 mb-4">
            <span className="font-semibold text-blue-600">{selectedStudent.name}</span>님, 
            설문 응답이 성공적으로 완료되었습니다!
          </p>
          <div className="text-sm text-gray-600 mb-6 space-y-2">
            <p className="flex items-center justify-center">
              <span className="mr-2">✅</span>
