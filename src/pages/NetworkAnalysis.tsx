...
const NetworkAnalysis: React.FC = () => {
  ...
  const [analysisResults, setAnalysisResults] = useState<NetworkAnalysisResult | null>(null);
  const [savedAnalysisList, setSavedAnalysisList] = useState<Array<{
    id: string;
    survey_id: string;
    survey_title: string;
  ...
  const connectionCounts = useMemo(() => {
    ...
    analysisResults.nodes.forEach((node) => {
      ...
      if (analysisResults.edges) {
        ...
        const outgoingEdges = analysisResults.edges.filter(edge => edge.source === node.id).length;
  ...
  const fetchSavedAnalysisList = async () => {
    try {
      ...
      // null 값 필터링
      const filteredAnalysisList = analysisList.filter(item => item !== null) as Array<{
        id: string;
        survey_id: string;
        survey_title: string;
        calculated_at: string;
  ...
  const loadSavedNetworkAnalysis = async (surveyId: string): Promise<NetworkAnalysisResult | null> => {
    try {
  ...
  const handleRunNetworkAnalysis = async () => {
    ...
    try {
      ...
      // Python 백엔드 호출 (시뮬레이션)
      const mockAnalysisResults = await simulatePythonAnalysis(selectedSurvey.id);
  ...
  const saveNetworkAnalysisToDB = async (surveyId: string, analysisResults: NetworkAnalysisResult): Promise<void> => {
    try {
      ...
      const completeAnalysisRecord = {
        ...
        risk_indicators: {
          ...
          average_centrality: analysisResults.nodes.reduce((sum, n) => sum + n.centrality, 0) / analysisResults.nodes.length
        },
        recommendations: {
  ...
  // Python 분석 시뮬레이션
  const simulatePythonAnalysis = async (surveyId: string): Promise<NetworkAnalysisResult> => {
    ...
    try {
      ...
      if (error) {
        ...
        console.log('🔍 실제 설문 응답 데이터 기반 엣지 생성:', surveyResponses.length);
                ...
                questionResponses.forEach((friendId: string) => {
                  if (friendId && friendId !== studentId) {
                    ...
                    const existingEdge = edges.find(edge => 
                      (edge.source === studentId && edge.target === friendId) ||
    ...
    const metrics: NetworkMetrics = {
      ...
      density: students.length > 1 ? edges.length / (students.length * (students.length - 1)) : 0,
      average_degree: students.length > 0 ? (edges.length * 2) / students.length : 0,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow mb-6">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-medium text-gray-900">분석할 설문 선택</h3>
              <p className="text-sm text-gray-600">교우관계 분석을 위한 설문을 선택해주세요.</p>
            </div>
            {teacherInfo && (
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              <div className="text-sm text-gray-600 mb-4">
                총 <span className="font-semibold text-blue-600">{surveys.length}개</span>의 설문을 찾았습니다.
              </div>
                ...
                {surveys.map((survey) => (
                  <div
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const savedResults = await loadSavedNetworkAnalysis(survey.id);
                        if (savedResults) {
                          console.log('🔍 설문 선택 시 저장된 분석 결과를 불러왔습니다');
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    {survey.target_grades && survey.target_grades.length > 0 ? (
                      <div>
                        <span className="font-medium">대상:</span> {survey.target_grades.join(', ')}학년 {survey.target_classes && survey.target_classes.length > 0 ? survey.target_classes.join(', ') + '반' : ''}
                      </div>
                    ) : (
                  ...
                  <div className="space-y-1 text-sm text-gray-500 mt-2">
                    <div>생성일: {new Date(survey.created_at || '').toLocaleDateString()}</div>
                    {survey.start_date && (
                      <div>시작일: {new Date(survey.start_date).toLocaleDateString()}</div>
                    )}
                    {survey.end_date && (
                      <div>종료일: {new Date(survey.end_date).toLocaleDateString()}</div>
                    )}
                  </div>
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-lg font-medium mb-2">현재 담당 학급의 설문이 없습니다</p>
              <p className="text-sm mb-4">다른 학년/반의 설문이거나 아직 생성된 설문이 없을 수 있습니다.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="p-6 text-center">
            ...
            {analysisResults && (
              <div className="mt-3 pt-3 border-t border-gray-100 space-y-3">
                <p className="text-xs text-green-600 mb-2">✅ 분석 결과가 로드되었습니다</p>
                ...
                <div className="flex space-x-3 justify-center">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (analysisResults) {
                        localStorage.setItem('temp_network_analysis', JSON.stringify({
      ...
      {analysisView === 'overview' && analysisResults && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">네트워크 메트릭</h3>
              <p className="text-sm text-gray-600">전체 네트워크의 구조적 특성을 분석합니다.</p>
            </div>
            
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">{analysisResults.metrics.total_students}</div>
                  <div className="text-sm text-gray-600">총 학생 수</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{analysisResults.metrics.total_relationships}</div>
                  <div className="text-sm text-gray-600">총 관계 수</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">{analysisResults.metrics.average_degree.toFixed(1)}</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
                <div className="p-4 bg-blue-50 rounded-lg">
                  <div className="text-sm font-medium text-blue-900">네트워크 밀도</div>
                  <div className="text-lg font-semibold text-blue-700">{(analysisResults.metrics.density * 100).toFixed(1)}%</div>
                </div>
                <div className="p-4 bg-green-50 rounded-lg">
                  <div className="text-sm font-medium text-green-900">클러스터링 계수</div>
                  <div className="text-lg font-semibold text-green-700">{(analysisResults.metrics.clustering_coefficient * 100).toFixed(1)}%</div>
                </div>
                <div className="p-4 bg-purple-50 rounded-lg">
                  <div className="text-sm font-medium text-purple-900">평균 경로 길이</div>
                  <div className="text-lg font-semibold text-purple-700">{analysisResults.metrics.average_path_length.toFixed(1)}</div>
                </div>
                <div className="p-4 bg-orange-50 rounded-lg">
                  <div className="text-sm font-medium text-orange-900">모듈성</div>
                  <div className="text-lg font-semibold text-orange-700">{(analysisResults.metrics.modularity * 100).toFixed(1)}%</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">커뮤니티 분석</h3>
              <p className="text-sm text-gray-600">자연스럽게 형성된 친구 그룹을 분석합니다.</p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {analysisResults.communities.map((community) => (
                  <div key={community.id} className="border border-gray-200 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 mb-2">커뮤니티 {community.id + 1}</h4>
                    <div className="space-y-2 text-sm text-gray-600">
                      <div>멤버 수: {community.size}명</div>
                      <div>내부 밀도: {(community.internal_density * 100).toFixed(1)}%</div>
                      <div className="text-xs text-gray-500">
                        멤버: {community.members.slice(0, 3).map(id => 
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">학생별 중심성 분석</h3>
              <p className="text-sm text-gray-600">각 학생의 네트워크 내 영향력과 연결성을 분석합니다.</p>
            </div>
                      ...
                      <div key={node.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            friendshipType === "사교 스타" ? "bg-purple-100 text-purple-800" :
                            friendshipType === "친구 많은 학생" ? "bg-green-100 text-green-800" :
                            friendshipType === "평균적인 학생" ? "bg-blue-100 text-blue-800" :
                            friendshipType === "소수 친구 학생" ? "bg-yellow-100 text-yellow-800" :
                        ...
                        <div className="space-y-2 text-sm text-gray-600">
                          ...
                          <div className="flex justify-between">
                            <span>커뮤니티:</span>
                            <span className="font-medium">{node.community + 1}번</span>
                          </div>
                        </div>
                      </div>
                    );
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">네트워크 구조 분석</h3>
              <p className="text-sm text-gray-600">네트워크의 전반적인 구조와 특성을 분석합니다.</p>
            </div>
                  ...
                  <div className="space-y-3">
                    {(() => {
                      const connectionRanges = [
                        { min: 1, max: 2, label: "1-2명", color: "bg-red-100 text-red-800" },
                        { min: 3, max: 4, label: "3-4명", color: "bg-yellow-100 text-yellow-800" },
                        { min: 5, max: 6, label: "5-6명", color: "bg-green-100 text-green-800" },
                        { min: 7, max: 10, label: "7명 이상", color: "bg-blue-100 text-blue-800" }
                      ];
                      
                      return connectionRanges.map((range) => {
                        const count = Array.from(connectionCounts.values()).filter(
                          c => c >= range.min && c <= range.max
                        ).length;
                        
                        const percentage = analysisResults.nodes.length > 0 ? 
                          (count / analysisResults.nodes.length * 100).toFixed(1) : '0.0';
                              ...
                              <div className="w-24 bg-gray-200 rounded-full h-2">
                                <div 
                                  className={`h-2 rounded-full ${
                                    range.color.includes('red') ? 'bg-red-500' :
                                    range.color.includes('yellow') ? 'bg-yellow-500' :
                                    range.color.includes('green') ? 'bg-green-500' : 'bg-blue-500'
                                  }`}
                  ...
                  <div className="space-y-3">
                    {(() => {
                      const centralityRanges = [
                        { min: 0, max: 0.2, label: "0-20%", color: "bg-red-100 text-red-800" },
                        { min: 0.2, max: 0.4, label: "20-40%", color: "bg-yellow-100 text-yellow-800" },
                        { min: 0.4, max: 0.6, label: "40-60%", color: "bg-blue-100 text-blue-800" },
                        { min: 0.6, max: 0.8, label: "60-80%", color: "bg-green-100 text-green-800" },
                        { min: 0.8, max: 1.0, label: "80-100%", color: "bg-purple-100 text-purple-800" }
                      ];
                      
                      return centralityRanges.map((range) => {
                        const count = analysisResults.nodes.filter(
                          n => n.centrality >= range.min && n.centrality < range.max
                        ).length;
                              ...
                              <div className="w-24 bg-gray-200 rounded-full h-2">
                                <div 
                                  className={`h-2 rounded-full ${
                                    range.color.includes('red') ? 'bg-red-500' :
                                    range.color.includes('yellow') ? 'bg-yellow-500' :
                                    range.color.includes('blue') ? 'bg-blue-500' :
                                    range.color.includes('green') ? 'bg-green-500' : 'bg-purple-500'
                                  }`}
                                  style={{ width: `${percentage}%` }}
                                ></div>
                              </div>
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">개선 권장사항</h3>
              <p className="text-sm text-gray-600">네트워크 분석 결과를 바탕으로 한 개선 방안을 제시합니다.</p>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  <h4 className="font-medium text-blue-900 mb-3">친구관계 발전</h4>
                  <ul className="list-disc list-inside space-y-2 text-sm text-blue-800">
                    <li>연결 수가 적은 학생들을 위한 그룹 활동 프로그램 운영</li>
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <h4 className="font-medium text-green-900 mb-3">커뮤니티 통합</h4>
      ...
      {analysisView === 'individual' && analysisResults && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">개별 관계 분석 요약</h3>
              <p className="text-sm text-gray-600">전체 학생의 네트워크 관계를 종합적으로 분석합니다.</p>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">
                    {analysisResults.nodes.filter(n => n.centrality >= 0.6).length}
                  </div>
                ...
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">
                    {analysisResults.nodes.filter(n => n.centrality >= 0.3 && n.centrality < 0.6).length}
                  </div>
                ...
                <div className="text-center p-4 bg-yellow-50 rounded-lg">
                  <div className="text-2xl font-bold text-yellow-600">
                    {analysisResults.nodes.filter(n => n.centrality < 0.3).length}
                  </div>
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-medium text-gray-900">학생별 상세 분석</h3>
                  <p className="text-sm text-gray-600">각 학생의 네트워크 중심성과 연결 관계를 분석합니다.</p>
                </div>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                ...
                <button
                  onClick={() => setAnalysisResults({
                    ...analysisResults,
                    nodes: [...analysisResults.nodes].sort((a, b) => b.centrality - a.centrality)
                  })}
                ...
                <button
                  onClick={() => setAnalysisResults({
                    ...analysisResults,
                    nodes: [...analysisResults.nodes].sort((a, b) => 
                ...
                <button
                  onClick={() => setAnalysisResults({
                    ...analysisResults,
                    nodes: [...analysisResults.nodes].sort((a, b) => a.community - b.community)
                  })}
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">커뮤니티별 분석</h3>
              <p className="text-sm text-gray-600">각 커뮤니티의 특성과 구성원을 분석합니다.</p>
            </div>
            
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {analysisResults.communities.map((community) => {
                  const communityStudents = analysisResults.nodes.filter(n => n.community === community.id);
                  const avgCentrality = communityStudents.reduce((sum, n) => sum + n.centrality, 0) / communityStudents.length;
                  
                  return (
                    <div key={community.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-gray-900">커뮤니티 {community.id + 1}</h4>
                      ...
                      <div className="space-y-2 text-sm text-gray-600">
                        <div className="flex justify-between">
                          <span>평균 중심성:</span>
                          <span className="font-medium">{(avgCentrality * 100).toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span>내부 밀도:</span>
                          <span className="font-medium">{(community.internal_density * 100).toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span>전체 비율:</span>
                          <span className="font-medium">{((community.size / analysisResults.nodes.length) * 100).toFixed(1)}%</span>
                        </div>
                      </div>
                      
                      <div className="mt-3 pt-3 border-t border-gray-100">
                        <div className="text-xs text-gray-500 mb-2">주요 구성원:</div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-gray-900">현재 교우관계 네트워크</h3>
                  <p className="text-sm text-gray-600 mt-1">학생들 간의 친구 관계를 시각화하여 네트워크 구조를 분석합니다</p>
                </div>
                <div className="text-right">
                  <div className="text-sm text-gray-500">총 학생 수</div>
                  <div className="text-lg font-semibold text-blue-600">{analysisResults.nodes.length}명</div>
                  <div className="text-sm text-gray-500">총 관계 수</div>
                  <div className="text-lg font-semibold text-green-600">{analysisResults.edges.length}개</div>
                </div>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-xs">
                      ...
                      <div className="text-gray-600">
                        {analysisResults.nodes.filter(n => n.centrality < 0.2).length}명
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-xs">
                      ...
                      <div className="text-gray-600">
                        {analysisResults.nodes.filter(n => n.centrality >= 0.2 && n.centrality < 0.4).length}명
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-xs">
                      ...
                      <div className="text-gray-600">
                        {analysisResults.nodes.filter(n => n.centrality >= 0.4 && n.centrality < 0.6).length}명
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-xs">
                      ...
                      <div className="text-gray-600">
                        {analysisResults.nodes.filter(n => n.centrality >= 0.6 && n.centrality < 0.8).length}명
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="text-xs">
                      ...
                      <div className="text-gray-600">
                        {analysisResults.nodes.filter(n => n.centrality >= 0.8).length}명
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="text-lg font-bold text-blue-600">
                    {analysisResults.nodes.filter(n => n.centrality < 0.2).length}
                  </div>
                  <div className="text-xs text-blue-800">외톨이형</div>
                </div>
                <div className="text-center p-3 bg-yellow-50 rounded-lg">
                  <div className="text-lg font-bold text-yellow-600">
                    {analysisResults.nodes.filter(n => n.centrality >= 0.2 && n.centrality < 0.4).length}
                  </div>
                  <div className="text-xs text-yellow-800">소수 친구</div>
                </div>
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="text-lg font-bold text-blue-600">
                    {analysisResults.nodes.filter(n => n.centrality >= 0.4 && n.centrality < 0.6).length}
                  </div>
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600">네트워크 시각화의 주요 통계 정보입니다.</p>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {analysisResults.nodes.filter(n => n.centrality >= 0.5).length}
                  </div>
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">커뮤니티 시각화</h3>
              <p className="text-sm text-gray-600">각 커뮤니티의 구성과 특성을 시각적으로 표현합니다.</p>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {analysisResults.communities.map((community) => {
                  ...
                  const textColors = ['text-red-800', 'text-blue-800', 'text-green-800', 'text-yellow-800', 'text-purple-800'];
                  
                  return (
                    <div key={community.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-gray-900">커뮤니티 {community.id + 1}</h4>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${colors[community.id % colors.length]} ${textColors[community.id % textColors.length]}`}>
                          {community.size}명
                        </span>
                      </div>
                        ...
                        {communityStudents.slice(0, 5).map((node) => {
                          ...
                          return (
                            <div key={node.id} className="flex items-center justify-between text-sm">
                              ...
                              <span className="text-gray-500">{(node.centrality * 100).toFixed(0)}%</span>
                            </div>
                          );
                        })}
      ...
      {!analysisResults && !analysisError && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-medium text-blue-900 mb-2">네트워크 분석 준비 완료</h3>
          <p className="text-blue-700">
            위에서 설문을 선택하고 "네트워크 분석 실행" 버튼을 클릭하여 교우관계 분석을 시작하세요.
          </p>
        </div>
      )}

      {/* 개별 학생 네트워크 모달 */}
      {selectedStudentModal.isOpen && selectedStudentModal.student && selectedStudentModal.node && analysisResults && (
          ...
          <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-6xl shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600">
                    {selectedStudentModal.student.grade}학년 {selectedStudentModal.student.class}반
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="text-sm font-medium text-blue-600">중심성 점수</div>
                  <div className="text-2xl font-bold text-blue-900">
                    {(selectedStudentModal.node.centrality * 100).toFixed(1)}%
          </div>
        </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="text-sm font-medium text-green-600">연결 수</div>
                ...
                <div className="bg-purple-50 p-4 rounded-lg">
                  <div className="text-sm font-medium text-purple-600">커뮤니티</div>
              ...
              {/* 네트워크 시각화 */}
              <div className="mb-6">
                <h4 className="text-lg font-medium text-gray-900 mb-4">연결 관계 네트워크</h4>
                  ...
                  <NetworkVisualization 
                    data={{
                      nodes: (() => {
                        ...
                        const connectedStudentIds = new Set([selectedStudentId, ...actualConnections]);
              ...
              {/* 직접 연결된 친구들 */}
              <div className="mb-6">
                <h4 className="text-lg font-medium text-gray-900 mb-4">직접 연결된 친구들</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {(() => {
                    ...
                    // 실제 연결된 학생들만 표시
                    const connectedStudents = analysisResults.nodes.filter(node => 
                      actualConnections.includes(node.id)
                    );
                      ...
                      return (
                        <div key={index} className="border border-gray-200 rounded-lg p-3 bg-white">
                          ...
                          <div className="text-sm text-gray-600 space-y-1">
                            <div>학년/반: {student.grade}학년 {student.class}반</div>
                            <div>중심성: {(node.centrality * 100).toFixed(1)}%</div>
                            <div>커뮤니티: {node.community + 1}번</div>
                            <div>관계 강도: {Math.floor(Math.random() * 5) + 1}/5</div>
                          </div>
                        </div>
                      );
                    });
                ...
                {(() => {
                  ...
                  if (actualConnections.length === 0) {
                    return (
                      <div className="text-center py-8 text-gray-500">
                        <div className="text-lg font-medium mb-2">연결된 친구가 없습니다</div>
                        <div className="text-sm">이 학생은 현재 설문에서 다른 학생을 친구로 선택하지 않았습니다.</div>
                      </div>
                    );
                  }
                  return null;
              ...
              {/* 네트워크 메트릭 */}
              <div className="mb-6">
                <h4 className="text-lg font-medium text-gray-900 mb-4">네트워크 메트릭</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-sm font-medium text-gray-600">연결 밀도</div>
                    <div className="text-lg font-semibold text-gray-900">
                      {(() => {
                        ...
                        const connectionDensity = (actualConnections.length / Math.max(students.length - 1, 1)) * 100;
                        return connectionDensity.toFixed(1);
                      })()}%
                    </div>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-sm font-medium text-gray-600">평균 연결 거리</div>
                    <div className="text-lg font-semibold text-gray-900">
              ...
              {/* 개인별 요약 */}
              <div className="mb-6">
                <h4 className="text-lg font-medium text-gray-900 mb-4">개인별 요약</h4>
                
                {/* 1. 현재 상태 */}
                <div className="mb-4">
                  <h5 className="font-medium text-gray-900 mb-2">1. 현재 상태</h5>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <ul className="list-disc list-inside space-y-1 text-sm text-gray-700">
                      <li>네트워크 중심성: {(selectedStudentModal.node.centrality * 100).toFixed(1)}%</li>
                      <li>직접 연결된 친구 수: {(() => {
                        const selectedStudentId = selectedStudentModal.student!.id;
                        const actualConnections = analysisResults.edges
                          .filter(edge => edge.source === selectedStudentId)
                      ...
                      <li>소속 커뮤니티: {selectedStudentModal.node.community + 1}번 그룹</li>
                      <li>네트워크 내 위치: {selectedStudentModal.node.centrality < 0.3 ? '주변부' : selectedStudentModal.node.centrality < 0.6 ? '중간' : '중심부'}</li>
                    </ul>
                  </div>
                </div>
                ...
                {/* 2. 네트워크 안정성 */}
                <div className="mb-4">
                  <h5 className="font-medium text-gray-900 mb-2">2. 네트워크 안정성</h5>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <ul className="list-disc list-inside space-y-1 text-sm text-gray-700">
                      <li>연결 밀도: {(() => {
                        const selectedStudentId = selectedStudentModal.student!.id;
                      ...
                      <li>안정성 지수: {selectedStudentModal.node.centrality < 0.3 ? '낮음' : selectedStudentModal.node.centrality < 0.6 ? '보통' : '높음'}</li>
                      <li>커뮤니티 통합도: {selectedStudentModal.node.community >= 0 ? '양호' : '개선 필요'}</li>
                      <li>네트워크 영향력: {selectedStudentModal.node.centrality < 0.3 ? '제한적' : selectedStudentModal.node.centrality < 0.6 ? '보통' : '높음'}</li>
                    </ul>
                  </div>
                </div>
                ...
                {/* 3. 개선방안 */}
                <div className="mb-4">
                  <h5 className="font-medium text-gray-900 mb-2">3. 개선방안</h5>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <ul className="list-disc list-inside space-y-1 text-sm text-gray-700">
                ...
                {/* 4. 그룹 소속 요약 */}
                <div>
                  <h5 className="font-medium text-gray-900 mb-2">4. 그룹 소속 요약</h5>
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-blue-800 font-medium">커뮤니티 {selectedStudentModal.node?.community !== undefined ? selectedStudentModal.node.community + 1 : 0}번 그룹</span>
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                        {analysisResults.communities.find(c => c.id === (selectedStudentModal.node?.community || 0))?.size || 0}명
                      </span>
                    </div>
                    <div className="text-sm text-blue-700">
                      <p>• 그룹 내부 밀도: {(analysisResults.communities.find(c => c.id === selectedStudentModal.node?.community)?.internal_density || 0) * 100}%</p>
                      <p>• 그룹 내 역할: {selectedStudentModal.node && selectedStudentModal.node.centrality < 0.3 ? '참여자' : selectedStudentModal.node && selectedStudentModal.node.centrality < 0.6 ? '활동가' : '리더'}</p>
                      <p>• 그룹 기여도: {selectedStudentModal.node && selectedStudentModal.node.centrality < 0.3 ? '개선 필요' : selectedStudentModal.node && selectedStudentModal.node.centrality < 0.6 ? '양호' : '우수'}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* 권장사항 */}
              <div>
                <h4 className="text-lg font-medium text-gray-900 mb-4">개선 권장사항</h4>
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="text-sm text-yellow-800">
                    {selectedStudentModal.node.centrality < 0.3 ? (
                      <div>
                        <p className="font-medium mb-2">⚠️ 주의가 필요한 학생</p>
                      ...
                      <div>
                        <p className="font-medium mb-2">📈 개선 여지가 있는 학생</p>
                        <ul className="list-disc list-inside space-y-1">
                      ...
                      <div>
                        <p className="font-medium mb-2">🌟 우수한 네트워크를 가진 학생</p>
                        <ul className="list-disc list-inside space-y-1">
