...
import { NetworkAnalysisData, NetworkMetrics } from "../types";
...
const NetworkAnalysisPage: React.FC = () => {
  ...
  const handleNodeClick = useCallback(
    (node: any) => {
      ...
      // 해당 학생의 데이터 찾기
      const studentData = chartData[activeTab - 1]?.nodes.find(
        (n: any) => n.id === studentId,
      );
      if (studentData) {
      ...
      try {
        ...
        if (
          ...
          if (filteredSurveys.length > 0) {
            ...
            const projectData: SurveyProject[] = filteredSurveys.map(
              (survey: any) => ({
                pid: survey.id,
                name: survey.title,
                ...
                response_count: 0,
                template_category:
                  survey.survey_templates?.metadata?.category || "분석가능",
              }),
            );
    ...
    try {
      ...
      for (const item of selectedItems) {
        try {
          ...
          // 외톨이형 수 계산
          const isolatedCount = result.nodes.filter(
            (node: any) => node.friendship_type === "외톨이형",
          ).length;
          ...
          analysisResults.push({
            ...
            metrics: {
              ...
              clusterCount: Math.max(
                0,
                (result.metrics?.connected_components || 0) - isolatedCount,
              ),
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center space-x-2 text-xs text-gray-600">
                          <span>
                            템플릿: {item.template_category || "분석가능"}
                          </span>
                          <span>•</span>
