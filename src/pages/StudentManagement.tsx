...
const StudentManagement: React.FC = () => {
  ...
  const getAccessScope = () => {
    ...
    switch (teacherInfo.role) {
      ...
      try {
        ...
        } else if (teacherData && teacherData.grade_level && teacherData.class_number) {
        ...
        console.log('🔍 StudentManagement 사용자 정보 설정 완료:', { user, teacherData });
      } catch (error) {
  ...
  const fetchStudents = async () => {
    try {
      ...
      if (teacherInfo?.role === 'homeroom_teacher' && teacherInfo.grade_level && teacherInfo.class_number) {
        // 담임교사: 담당 학년/반만
        console.log('담임교사 필터링 적용:', teacherInfo.grade_level, '학년', teacherInfo.class_number, '반');
        query = query
      ...
      if (studentsData && studentsData.length > 0) {
        ...
        console.log('학생들의 학년/반 분포:', studentsData.map(s => `${s.grade}학년${s.class}반`));
      }
      ...
      const studentsWithData = studentsData?.map(student => {
        ...
        if (networkData && networkData.length > 0) {
          ...
          if (completeData?.nodes) {
            const node = completeData.nodes.find((n: any) => 
              n.id === student.id || 
        ...
        const memos = memosData?.filter(m => m.student_id === student.id) || [];
        const interventions = interventionData?.filter(i => i.student_id === student.id) || [];
    ...
    const sampleStudents: Student[] = [
      {
        ...
        teacher_memos: [{ 
          id: '1', 
          content: '조용한 편이지만 친구들과의 관계는 양호함. 좀 더 관심을 가져볼 필요가 있음.',
  ...
  const getGradeOptions = () => {
    ...
    const grades = students.map(s => s.grade).filter((grade, index, arr) => arr.indexOf(grade) === index).sort((a, b) => parseInt(a) - parseInt(b));
    return grades;
  };

  const getClassOptions = () => {
    if (students.length === 0) return [];
    const classes = students.map(s => s.class).filter((cls, index, arr) => arr.indexOf(cls) === index).sort((a, b) => parseInt(a) - parseInt(b));
    return classes;
  };
  ...
  const getRiskLevel = (student: Student) => {
    ...
    const centrality = student.network_metrics.centrality_scores?.centrality || 
                      student.network_metrics.centrality_scores?.degree || 0;
  ...
  const getHighRiskStudentCount = () => {
    return students.filter(student => getRiskLevel(student) === 'high').length;
  };
  ...
  const convertExcelDate = (excelDate: any): string => {
    ...
    if (typeof excelDate === 'number' || !isNaN(Number(excelDate))) {
      ...
      const targetDate = new Date(excelStartDate.getTime() + (serialNumber - 1) * 24 * 60 * 60 * 1000);
    ...
    return [...students].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'network_centrality':
          aValue = a.network_metrics?.centrality_scores?.centrality || 
                   a.network_metrics?.centrality_scores?.degree || 0;
          bValue = b.network_metrics?.centrality_scores?.centrality || 
                   b.network_metrics?.centrality_scores?.degree || 0;
          break;
      }
    ...
    fileInput.onchange = async (event) => {
      ...
      try {
        ...
        if (fileExtension === 'csv') {
          ...
          toast.error('지원하지 않는 파일 형식입니다. CSV 또는 Excel 파일을 사용해주세요.');
        }
      } catch (error) {
  ...
  const handleCSVUpload = async (file: File) => {
    ...
    // 업로드 확인 다이얼로그
    if (window.confirm(`${students.length}명의 학생 정보를 업로드하시겠습니까?`)) {
  ...
  const processUploadedStudents = async (students: any[]) => {
    try {
      ...
      // 업로드 진행 상황 표시
      toast.loading(`${validStudents.length}명의 학생 정보를 업로드하는 중...`, { duration: 0 });
      ...
      for (let i = 0; i < validStudents.length; i++) {
        ...
        try {
          ...
          const studentData = {
            ...
            is_active: true,
            lifelong_education_id: student['교육ID'] || `LEI${new Date().getFullYear()}_${String(i + 1).padStart(6, '0')}`, // 엑셀에서 가져오거나 자동 생성
            current_school_id: teacherInfo?.school_id || null, // 담임선생님의 학교 ID로 자동 설정
            parent_contact: student['어머니_이름'] || student['어머니_전화번호'] || student['아버지_이름'] || student['아버지_전화번호'] ? {
          ...
          console.log('- student_number:', typeof studentData.student_number, studentData.student_number);
          console.log('- gender:', typeof studentData.gender, studentData.gender);
          console.log('- birth_date:', typeof studentData.birth_date, studentData.birth_date, '(원본:', student['생년월일'], ')');
          console.log('- enrolled_at:', typeof studentData.enrolled_at, studentData.enrolled_at, '(원본:', student['입학일'], ')');
          console.log('- is_active:', typeof studentData.is_active, studentData.is_active);
          console.log('- lifelong_education_id:', typeof studentData.lifelong_education_id, studentData.lifelong_education_id);
          console.log('- parent_contact:', typeof studentData.parent_contact, studentData.parent_contact);
          console.log('- created_at:', typeof studentData.created_at, studentData.created_at);
          console.log('- updated_at:', typeof studentData.updated_at, studentData.updated_at);
          ...
          if (insertError) {
            ...
            throw new Error(`${student['이름']} 저장 실패: ${insertError.message}`);
          }
      ...
      if (successfulUploads.length > 0) {
        ...
        if (failedUploads.length === 0) {
          toast.success(`${successfulUploads.length}명의 학생 정보가 성공적으로 업로드되었습니다.`);
        } else {
          toast.success(`${successfulUploads.length}명 업로드 성공, ${failedUploads.length}명 실패`);
        }
      }
      
      // 실패한 업로드가 있다면 상세 정보 표시
      if (failedUploads.length > 0) {
        const failedNames = failedUploads.map(f => f.originalData['이름']).join(', ');
    ...
    try {
      ...
      setStudents(prev => prev.map(student => 
        student.id === selectedStudent.id 
          ? { ...student, teacher_memos: [...(student.teacher_memos || []), newMemo] }
          : student
      ));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-gray-900 mb-2">학생 데이터 로딩 중...</p>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">학생 관리 페이지에 접근할 수 있는 권한이 없습니다.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              ...
              <button
                ...
                title={teacherInfo?.school_id ? `${schoolName || '현재 학교'}에 학생 등록` : '학교 정보가 없습니다'}
              >
        ...
        {/* 담임 정보 표시 */}
        {teacherInfo && (teacherInfo.role === 'homeroom_teacher' || teacherInfo.role === 'grade_teacher') && (
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-lg font-medium text-blue-900">
                    {teacherInfo.name || currentUser?.email} {getRoleDisplayName(teacherInfo.role)}
                  </h3>
                  <p className="text-sm text-blue-700">
                    {teacherInfo.grade_level}학년 {teacherInfo.class_number}반 담당
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded-lg text-sm text-gray-600">
            <div className="flex flex-wrap gap-4">
              ...
              <div>
                ...
                {(teacherInfo?.role === 'homeroom_teacher' || teacherInfo?.role === 'grade_teacher') && ' (담임 고정)'}
              </div>
              <div>
                <strong>표시 학생:</strong> {filteredStudents.length}명 / {students.length}명
              </div>
            </div>
            
            {/* 담임 정보 표시 */}
            {(teacherInfo?.role === 'homeroom_teacher' || teacherInfo?.role === 'grade_teacher') && teacherInfo.grade_level && teacherInfo.class_number && (
              <div className="mt-3 p-2 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
                🎯 {getRoleDisplayName(teacherInfo.role)}: {teacherInfo.grade_level}학년 {teacherInfo.class_number}반 - 담당 반 학생만 표시
              </div>
            )}
          </div>
          ...
          <div className="mb-6 p-4 bg-gradient-to-r from-red-50 to-yellow-50 border border-red-200 rounded-lg">
            <h3 className="text-lg font-semibold text-red-900 mb-3">📊 위험도별 학생 현황</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-red-100 border border-red-300 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-red-800">{getHighRiskStudentCount()}</div>
                <div className="text-sm text-red-700 font-medium">주의 필요</div>
                <div className="text-xs text-red-600">중심성 &lt; 0.3</div>
              </div>
              <div className="bg-yellow-100 border border-yellow-300 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-yellow-800">{getRiskLevelCounts().medium}</div>
                <div className="text-sm text-yellow-700 font-medium">관찰 중</div>
                <div className="text-xs text-yellow-600">중심성 0.3~0.6</div>
              </div>
              <div className="bg-green-100 border border-green-300 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-green-800">{getRiskLevelCounts().low}</div>
            ...
            <div className="mt-3 text-center text-sm text-red-700">
              🚨 <strong>주의 학생 수: {getHighRiskStudentCount()}명</strong> - 즉시 관찰 및 개입이 필요한 학생들
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select
              ...
              className={`px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                teacherInfo?.role === 'homeroom_teacher' ? 'bg-gray-100 cursor-not-allowed' : ''
              }`}
            >
        ...
        {isUploading && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-medium text-blue-900">학생 명단 업로드 중...</h3>
              <span className="text-sm text-blue-700">{uploadProgress} / {uploadTotal}</span>
            </div>
            
            {/* 학교 정보 표시 */}
            {teacherInfo?.school_id && (
              <div className="mb-3 p-2 bg-blue-100 border border-blue-300 rounded text-sm text-blue-800">
                <p><strong>등록 학교:</strong> {schoolName || '현재 학교'}</p>
                <p className="text-xs mt-1">모든 학생이 이 학교에 자동으로 등록됩니다.</p>
              </div>
            )}
            ...
            <p className="text-sm text-blue-600 mt-2">
              {uploadProgress}명의 학생 정보를 업로드했습니다. ({Math.round((uploadProgress / uploadTotal) * 100)}%)
            </p>
          </div>
        )}
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{student.name}</h3>
                          ...
                          {student.grade}학년 {student.class}반 {parseInt(student.student_number)}번
                        </p>
                    </div>
                  </div>
                  
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(getRiskLevel(student))}`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-gray-900">{selectedStudent.name} 상세 정보</h2>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-gray-900 font-medium text-blue-600">{parseInt(selectedStudent.student_number)}번</span>
                  </div>
                  <div>
                    <span className="text-gray-600">성별:</span>
                    <span className="ml-2 text-gray-900">{getGenderLabel(selectedStudent.gender)}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">교육 ID:</span>
                    <span className="ml-2 text-gray-900">{selectedStudent.lifelong_education_id}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">입학일:</span>
                    <span className="ml-2 text-gray-900">{new Date(selectedStudent.enrolled_at).toLocaleDateString()}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">학급:</span>
                    <span className="ml-2 text-gray-900">{selectedStudent.grade}학년 {selectedStudent.class}반</span>
                  </div>
                  <div>
                    <span className="text-gray-600">생년월일:</span>
                    <span className="ml-2 text-gray-900">{selectedStudent.birth_date}</span>
                  </div>
                </div>
              </div>
              ...
              {/* 학부모 연락처 */}
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">학부모 연락처</h3>
                {selectedStudent.parent_contact && typeof selectedStudent.parent_contact === 'object' ? (
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-gray-900">
                        {(() => {
                          try {
                            const value = (selectedStudent.parent_contact as any)?.mother_name;
                            return value !== undefined && value !== null ? value : '정보 없음';
                          } catch (error) {
                      ...
                      {(() => {
                        try {
                          const phone = (selectedStudent.parent_contact as any)?.mother_phone;
                          return phone && phone !== undefined && phone !== null ? (
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-gray-900">
                        {(() => {
                          try {
                            const value = (selectedStudent.parent_contact as any)?.father_name;
                            return value !== undefined && value !== null ? value : '정보 없음';
                          } catch (error) {
                      ...
                      {(() => {
                        try {
                          const phone = (selectedStudent.parent_contact as any)?.father_phone;
                          return phone && phone !== undefined && phone !== null ? (
                            <span className="ml-2 text-gray-600">({phone})</span>
                          ) : null;
                        } catch (error) {
                  ...
                  <p className="text-gray-500">등록된 학부모 연락처가 없습니다.</p>
                )}
              </div>

                                          {/* 교우관계 분석 */}
              {selectedStudent.network_metrics && typeof selectedStudent.network_metrics === 'object' && selectedStudent.network_metrics !== null && (
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">교우관계 분석</h3>
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-md font-medium text-gray-700 mb-2">중심성 지수</h4>
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="text-gray-600">연결 중심성:</span>
                          <span className="ml-2 text-gray-900">
                            {safeStringify((selectedStudent.network_metrics as any)?.centrality_scores?.centrality || 'N/A')}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-600">매개 중심성:</span>
                          <span className="ml-2 text-gray-900">
                            {safeStringify((selectedStudent.network_metrics as any)?.centrality_scores?.betweenness || 'N/A')}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-600">근접 중심성:</span>
                          <span className="ml-2 text-gray-900">
                            {safeStringify((selectedStudent.network_metrics as any)?.centrality_scores?.closeness || 'N/A')}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <span className="ml-2 text-gray-900">
                          {safeStringify((selectedStudent.network_metrics as any)?.community_membership || 'N/A')}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-600">네트워크 위험도:</span>
                        <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(getRiskLevel(selectedStudent))}`}>
                    ...
                    <div className="text-sm">
                      ...
                      <span className="ml-2 text-gray-900">
                                                   {selectedStudent.network_metrics?.centrality_scores?.centrality ?
                           (selectedStudent.network_metrics.centrality_scores.centrality * 100).toFixed(1) + '%' : 'N/A'}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-600">권장사항:</span>
                      <span className="ml-2 text-gray-900">
                        {safeStringify((selectedStudent.network_metrics as any)?.recommendations || 'N/A')}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                {selectedStudent.teacher_memos && Array.isArray(selectedStudent.teacher_memos) && selectedStudent.teacher_memos.length > 0 ? (
                  <div className="space-y-3">
                    {selectedStudent.teacher_memos.map((memo, index) => (
                      <div key={memo.id || index} className="bg-gray-50 p-4 rounded-lg">
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-sm text-gray-600">
                            {memo.created_at ? new Date(memo.created_at).toLocaleDateString() : '날짜 없음'}
                          </span>
                          {memo.teacher_name && (
                            <span className="text-sm text-gray-500">{memo.teacher_name}</span>
                          )}
                        </div>
                        <p className="text-gray-900">
                          {(() => {
                            try {
                              ...
                              return content !== undefined && content !== null ? content : '내용 없음';
                            } catch (error) {
      ...
      {memoModalOpen && selectedStudent && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full">
            {/* 모달 헤더 */}
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900">{selectedStudent.name} 학생 메모 추가</h2>
              <button
